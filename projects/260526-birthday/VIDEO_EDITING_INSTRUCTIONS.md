...
- Target duration for the current preview/final render is about 11 minutes 20 seconds after removing `ST-600`, `ST-604`, and `ST-614`, trimming `video_003_DJI_20000104171048_0015_D.MP4`, and keeping only the last 3 seconds of `video_005_DJI_20000104172535_0017_D.MP4`.
...
- The preview should be about 11 minutes 20 seconds. A small timing difference is acceptable if all content rules are satisfied.
...
- Preserve required images when deduplicating: keep `ST-716` as the final image, keep the title image, and keep manually placed images such as `ST-621`, `ST-709`, `ST-737`, `ST-738`, `ST-638`, `ST-608`, `ST-682`, and `ST-713`.
...
- For source video `video_016_4e1e990e-0b3c-404c-9fb0-ef25073073ea.mp4`, apply a persistent source-based audio focus rule: whenever this source video appears anywhere in the timeline, lower BGM volume to 50% of the normal BGM level and raise the original video audio to 2x of the normal original-audio level, capped at 1.0.
- The audio focus rule must follow the source video identity, not a fixed timestamp, so it remains active if the video is moved earlier or later in future timeline changes.
...
- Confirm the final preview/render is about 11 minutes 20 seconds long.
- Confirm the output has both video and BGM-mixed audio.
- Confirm the `video_016_4e1e990e-0b3c-404c-9fb0-ef25073073ea.mp4` source-based audio focus interval is present in the timeline report whenever that source video is used.
