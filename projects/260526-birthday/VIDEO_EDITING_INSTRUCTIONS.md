...
- In the first image slideshow around the 2-minute area, keep only the first four images after the opening video: `ST-630`, `ST-695`, `ST-700`, and `ST-703`.
- Move the last four images from that first slideshow to later slideshow groups while preserving their order: move `ST-709` and `ST-628` into the next image slideshow after `ST-608`, move `ST-701` into the following slideshow after `ST-699`, and move `ST-702` into the following slideshow after `ST-634`. Do not include `ST-621`.
- Move `ST-738`, `ST-737`, and `ST-627` out of the opening area and place them between `video_003_DJI_20000104171048_0015_D_clip_054_end` and `video_014_ST7_8341_clip_000_046`. Keep the order `ST-738`, `ST-737`, then `ST-627`, moving `ST-627` from the latest full-render `2:31` image position into the latest full-render `10:52` slideshow.
...
- Move `photo_013_ST-628`, `photo_038_ST-701`, and `photo_039_ST-702` out of the first 2-minute slideshow tail and into the next later slideshow groups as described above. Move `photo_018_ST-638` to immediately before `ST-667`.
...
- Move `ST-661`, the still image visible around latest full-render `14:53`, back to the original existing video cut instead of the artificial split. Place it between `video_006_DJI_20000104172624_0018_D.MP4` source clips `2:38.29-2:57` and `3:04-3:14.29`, with the normal dissolve on both sides. Do not split `3:14.29-3:48.29` just for `ST-661`. Keep `ST-690` later after `ST-706`.
...
- Preserve required images when deduplicating: keep `ST-716` as the final image, keep the title image, and keep manually placed images such as `ST-709`, `ST-737`, `ST-738`, `ST-627`, `ST-638`, `ST-608`, `ST-682`, `ST-713`, `ST-628`, `ST-701`, `ST-702`, `ST-634`, `ST-690`, `ST-686`, `ST-723`, `ST-688`, `ST-731`, `ST-729`, `ST-730`, and `ST-645`.
...
- Confirm the first 2-minute image slideshow contains only `ST-630`, `ST-695`, `ST-700`, and `ST-703`; confirm `ST-709` and `ST-628` are added to the next image slideshow, `ST-701` to the following slideshow, and `ST-702` to the following slideshow. Confirm `ST-638` appears immediately before `ST-667`.
...
- Confirm the still-image cluster around the prior `11:30` area is split: `ST-632` remains before the split `video_006` block, `ST-616` and `ST-625` are absent, `ST-661` is moved from the latest-full-render `14:53` artificial-split position and inserted back into the existing cut between `video_006...clip_158_3_177` and `video_006...clip_184_194_3`, and `ST-690` moves after `ST-706`.
...
- Confirm `ST-645`, `ST-646`, `ST-653`, `ST-686`, `ST-661`, and `ST-665` are inserted between the requested `video_006` split pieces, with `ST-646` between `clip_082_3_087` and `clip_131_153_3`, `ST-653` and `ST-686` consecutive, and `ST-661` between `clip_158_3_177` and `clip_184_194_3`.
...
- Confirm `ST-738`, `ST-737`, then `ST-627` appear between `video_003_DJI_20000104171048_0015_D_clip_054_end` and `video_014_ST7_8341_clip_000_046`, and are no longer in the opening image group.
