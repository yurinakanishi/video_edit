...
- Provides timeline workflow actions that build and validate `output\timelines\current.timeline.json` from the current project config and analysis reports, then export or execute audited FFmpeg command/filter-graph artifacts, including preview/proxy commands, from that validated timeline.
